...
export class QrController {
  ...
  @Get('user')
  @ApiOperation({
    summary: "Generate a QR code for the authenticated user's payment address",
  })
  @ApiResponse({
    status: 200,
    type: QrResponseDto,
    description: 'QR code data URL and payment URL',
  })
  @ApiResponse({ status: 400, description: 'Invalid query parameters' })
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 500, description: 'Internal server error' })
  async getUserQr(@Query() query: UserQrQueryDto, @Req() req: Request) {
  ...
  @ApiResponse({ status: 500, description: 'Internal server error' })
  async getPayLinkQr(@Param('tokenId') tokenId: string) {
