...
export class SettlementsService {
  ...
  async initiateSettlement(payment: Payment): Promise<void> {
    ...
    if (!settlement.requiresApproval) {
      ...
      await this.adminAlerts.raise({
        ...
        metadata: { merchantId: saved.merchantId, paymentId: payment.id, amount: netUsd },
        thresholdValue: 1,
      });
    }
  }
  ...
  private async executeFiatTransfer(settlement: Settlement, payment: Payment): Promise<void> {
    ...
    try {
      ...
      await this.adminAlerts.raise({
        ...
        metadata: { merchantId: settlement.merchantId, paymentId: payment.id },
        thresholdValue: 1,
      });
  ...
  async handlePartnerCallback(payload: PartnerCallbackPayload): Promise<void> {
    ...
    const payment = await this.paymentsRepo.findOne({ where: { settlementId: settlement.id } });
  ...
  async findAllAdmin(query: AdminSettlementsQueryDto) {
    ...
    if (partnerReference) where.partnerReference = partnerReference;
    if (bankReference) where.bankReference = bankReference;
    if (startDate && endDate) where.createdAt = Between(new Date(startDate), new Date(endDate));
    else if (startDate) where.createdAt = Between(new Date(startDate), new Date());
    else if (endDate) where.createdAt = Between(new Date('1970-01-01'), new Date(endDate));
  ...
  async retrySettlement(id: string): Promise<{ success: boolean; message: string }> {
    const settlement = await this.settlementsRepo.findOne({ where: { id }, relations: ['payments'] });
    if (!settlement) return { success: false, message: 'Settlement not found' };
    if (settlement.status !== SettlementStatus.FAILED) return { success: false, message: 'Only failed settlements can be retried' };
    ...
    if (settlement.payments.length > 0) await this.executeFiatTransfer(settlement, settlement.payments[0]);
  ...
  async approveSettlement(id: string): Promise<{ success: boolean; message: string }> {
    const settlement = await this.settlementsRepo.findOne({ where: { id }, relations: ['payments'] });
    if (!settlement) return { success: false, message: 'Settlement not found' };
    if (settlement.status !== SettlementStatus.PENDING_APPROVAL) return { success: false, message: 'Only settlements pending approval can be approved' };
    if (!settlement.requiresApproval) return { success: false, message: 'Settlement does not require manual approval' };

    settlement.status = SettlementStatus.PROCESSING;
    settlement.approvedAt = new Date();
    settlement.approvedBy = 'admin';
    await this.settlementsRepo.save(settlement);
